...
FORGETTING_METHOD = "finite_horizon_forgetting_grid"
MAX_FORGETTING_GRID_STATES = 300_000
...
def forgetting_count_bound(decay: float, horizon: int) -> float:
    """Largest discounted pseudo-count reachable over a finite horizon."""
    return float((1.0 - decay**horizon) / (1.0 - decay))


def posterior_mean_from_pseudo_counts(successes: float, failures: float) -> float:
    return float((successes + 1.0) / (successes + failures + 2.0))


def forgetting_interpolation_terms(
    ...
    success_scaled = np.clip(success_values, 0.0, max_count) / grid_step
    failure_scaled = np.clip(failure_values, 0.0, max_count) / grid_step

    success_low = np.floor(success_scaled).astype(int)
    failure_low = np.floor(failure_scaled).astype(int)
    success_low = np.clip(success_low, 0, n_grid - 1)
    failure_low = np.clip(failure_low, 0, n_grid - 1)
    success_high = np.minimum(success_low + 1, n_grid - 1)
    failure_high = np.minimum(failure_low + 1, n_grid - 1)

    success_weight = success_scaled - success_low
    failure_weight = failure_scaled - failure_low
    success_weight = np.where(success_high == success_low, 0.0, success_weight)
    failure_weight = np.where(failure_high == failure_low, 0.0, failure_weight)

    idx_ll = success_low * n_grid + failure_low
    idx_hl = success_high * n_grid + failure_low
    idx_lh = success_low * n_grid + failure_high
    idx_hh = success_high * n_grid + failure_high
    weight_ll = (1.0 - success_weight) * (1.0 - failure_weight)
    weight_hl = success_weight * (1.0 - failure_weight)
    weight_lh = (1.0 - success_weight) * failure_weight
    weight_hh = success_weight * failure_weight
    return (
        idx_ll,
        idx_hl,
        idx_lh,
...
def interpolate_forgetting_values(
    values: np.ndarray,
    terms: tuple[np.ndarray, ...],
) -> np.ndarray:
    idx_ll, idx_hl, idx_lh, idx_hh, weight_ll, weight_hl, weight_lh, weight_hh = terms
    return (
        weight_ll * values[idx_ll]
        + weight_hl * values[idx_hl]
        + weight_lh * values[idx_lh]
        + weight_hh * values[idx_hh]
    )


def interpolate_forgetting_scalar(
    values: np.ndarray,
    successes: float,
    failures: float,
    grid: dict,
) -> float:
    terms = forgetting_interpolation_terms(
        np.array([successes], dtype=float),
        np.array([failures], dtype=float),
    ...
    return float(interpolate_forgetting_values(values, terms)[0])


def build_forgetting_grid(decay: float, horizon: int, grid_step: float) -> dict:
    raw_bound = forgetting_count_bound(decay, horizon)
    n_grid = int(np.ceil(raw_bound / grid_step)) + 1
    state_count = n_grid * n_grid
    if state_count > MAX_FORGETTING_GRID_STATES:
        raise ValueError(
            "Forgetting grid would create "
    ...
    no_choice_successes = decay * successes
    no_choice_failures = decay * failures
    success_successes = decay * successes + 1.0
    success_failures = decay * failures
    failure_successes = decay * successes
    failure_failures = decay * failures + 1.0
    ...
    return {
        ...
        "S": successes,
        "F": failures,
        "no_choice_index": project(no_choice_successes, no_choice_failures),
        "success_index": project(success_successes, success_failures),
        "failure_index": project(failure_successes, failure_failures),
        "no_choice_terms": forgetting_interpolation_terms(
            no_choice_successes,
            no_choice_failures,
            n_grid=n_grid,
            max_count=max_count,
        ...
        "success_terms": forgetting_interpolation_terms(
            success_successes,
            success_failures,
            n_grid=n_grid,
            max_count=max_count,
        ...
        "failure_terms": forgetting_interpolation_terms(
            failure_successes,
            failure_failures,
            n_grid=n_grid,
            max_count=max_count,
...
def solve_finite_horizon_forgetting_approx(
    p0: float,
    params: ModelParams,
    horizon: int,
    early_periods: int,
    ...
    """Approximate finite-horizon DP with exponentially discounted beliefs."""
    grid = build_forgetting_grid(decay, horizon, grid_step)
    successes = grid["S"]
    failures = grid["F"]
    n_states = len(successes)
    demand = beta_ccdf(p0, successes, failures)
    next_value = np.zeros(n_states, dtype=float)
    current_value = np.zeros(n_states, dtype=float)
    policy_by_period: dict[int, np.ndarray] = {}
    ...
    no_choice_terms = grid["no_choice_terms"]
    success_terms = grid["success_terms"]
    failure_terms = grid["failure_terms"]

    demand_weight_sum = float(np.sum(demand))
    for period in range(horizon, 0, -1):
        continuation_no_choice = interpolate_forgetting_values(
            next_value,
            no_choice_terms,
        )
        continuation_success = interpolate_forgetting_values(
            next_value,
            success_terms,
        )
        continuation_failure = interpolate_forgetting_values(
            next_value,
            failure_terms,
        )
        continuation_gap = continuation_success - continuation_failure
        ...
        q1 = (
            ...
            * (
            ...
            + (1.0 - demand) * continuation_no_choice
        )
        q2 = (
            demand
            * (
        ...
        if period <= early_periods:
            q_gap_by_period[period] = (q2 - q1).astype(np.float32)
            policy_by_period[period] = np.where(use_product2, 2, 1).astype(np.int8)
            usage_rows.append(
                {
                    ...
                    "max_effective_count": grid["max_count"],
                    "product_2_share": float(np.mean(use_product2)),
                    "demand_weighted_product_2_share": (
                        float(np.sum(use_product2 * demand) / demand_weight_sum)
...
def summarize_forgetting_solution(solution: dict) -> dict:
    ...
    return {
        ...
        "state_count": int(usage["state_count"].iloc[0]),
        "max_effective_count": float(usage["max_effective_count"].iloc[0]),
        ...
        "early_product_2_share_mean": float(usage["product_2_share"].mean()),
        "early_demand_weighted_product_2_share_mean": float(
            usage["demand_weighted_product_2_share"].mean()
        ),
    }


def simulate_forgetting_policy(
    solution: dict,
    params: ModelParams,
    n_rep: int,
    periods: int,
    ...
    max_count = float(grid["max_count"])
    policy_by_period: dict[int, np.ndarray] = solution["policy_by_period"]
    q_gap_by_period: dict[int, np.ndarray] = solution.get("q_gap_by_period", {})
    records = []
    ...
    for rep in range(n_rep):
        ...
        for period in range(1, periods + 1):
            ...
            posterior_mean_t = posterior_mean_from_pseudo_counts(
                current_successes,
                current_failures,
            )
            q_gap_t = np.nan
            if period in q_gap_by_period:
                q_gap_t = interpolate_forgetting_scalar(
                    q_gap_by_period[period],
                    current_successes,
                    current_failures,
                    grid,
                )
                action = 2 if q_gap_t > params.tol else 1
            else:
            ...
            records.append(
                {
                    ...
                    "effective_observations": current_successes + current_failures,
                    "posterior_mean": posterior_mean_t,
                    "rho": demand_prob,
                    "q_gap_product2_minus_product1": q_gap_t,
                    "user_chose_A": int(user_chose_A),
                    "action_if_A": action,
                    "success": float(success_value) if user_chose_A else np.nan,
                    "profit": profit,
    ...
    paths["policy_recommends_product_2"] = (paths["action_if_A"] == 2).astype(float)
    time_summary = (
        paths.groupby(
            ["method", "p0", "forgetting_decay", "grid_step", "T", "N_or_T", "t"],
            observed=True,
        )
        .agg(
            average_profit_to_date=("average_profit_to_date", "mean"),
            ...
            posterior_mean=("posterior_mean", "mean"),
            effective_observations=("effective_observations", "mean"),
            rho=("rho", "mean"),
            A_market_share=("user_chose_A", "mean"),
            policy_recommends_product_2=("policy_recommends_product_2", "mean"),
            product_2_frequency=("product_2_when_A_chosen", "mean"),
...
def plot_posterior_density_panels(
    ...
    for ax_idx, (ax, group) in enumerate(zip(axes, groups, strict=False)):
        ...
        if "forgetting_decay" in group:
            subset = subset[
                np.isclose(subset["forgetting_decay"], group["forgetting_decay"])
            ]
        if subset.empty:
            ax.set_visible(False)
            continue
...
def forgetting_density_groups(paths: pd.DataFrame) -> tuple[list[dict], str]:
    p0_values = np.array(sorted(paths["p0"].unique()))
    decays = np.array(sorted(paths["forgetting_decay"].unique()))
    if len(decays) == 1:
        groups = [
            {
                "p0": float(p0),
                "forgetting_decay": float(decays[0]),
                "label": f"p0={p0:.2f}",
            }
            for p0 in selected_p0_values(paths)
        ]
        return groups, f"lambda={decays[0]:.3f}"
        ...
        groups = [
            {
                ...
                "forgetting_decay": float(decay),
                "label": f"lambda={decay:.3f}",
            }
            for decay in selected_decays
        ]
        return groups, f"p0={p0_values[0]:.2f}"
    ...
    return groups, f"p0={target_p0:.2f}"


def plot_forgetting_posterior_density_evolution(
    paths: pd.DataFrame,
    outputs_dir: Path,
) -> None:
    if paths.empty:
        return
    groups, subtitle = forgetting_density_groups(paths)
    plot_posterior_density_panels(
        paths=paths,
        outputs_dir=outputs_dir,
        groups=groups,
        success_column="effective_S",
        failure_column="effective_F",
        filename="finite_horizon_forgetting_posterior_density_evolution.png",
        title=f"Forgetting user posterior density evolution ({subtitle})",
    )


def forgetting_plot_series(time_summary: pd.DataFrame) -> tuple[pd.DataFrame, str]:
    p0_values = np.array(sorted(time_summary["p0"].unique()))
    decays = np.array(sorted(time_summary["forgetting_decay"].unique()))
    if len(decays) == 1:
        selected_p0 = selected_p0_values(time_summary)
        data = time_summary[time_summary["p0"].isin(selected_p0)].copy()
        data["series_label"] = data["p0"].map(lambda value: f"p0={value:.2f}")
        return data, f"lambda={decays[0]:.3f}"

    if len(p0_values) == 1:
        selected_decays = decays[: min(5, len(decays))]
        data = time_summary[time_summary["forgetting_decay"].isin(selected_decays)].copy()
        data["series_label"] = data["forgetting_decay"].map(
            lambda value: f"lambda={value:.3f}"
        )
    ...
    selected_decays = decays[: min(5, len(decays))]
    data = time_summary[
        np.isclose(time_summary["p0"], target_p0)
        & time_summary["forgetting_decay"].isin(selected_decays)
    ].copy()
    data["series_label"] = data["forgetting_decay"].map(
        lambda value: f"lambda={value:.3f}"
    )
...
def plot_forgetting_simulation_comparison(
    time_summary: pd.DataFrame,
    outputs_dir: Path,
) -> None:
    if time_summary.empty:
        return
    plot_data, subtitle = forgetting_plot_series(time_summary)
    ...
    for label in labels:
        ...
        profit_column = (
            ...
            if "avg_profit_per_period" in data.columns
            else "simulated_avg_profit"
        )
        ax_share.plot(
            data["t"],
            data["A_market_share"],
    ...
    fig.suptitle(
        f"Finite-horizon forgetting experiment ({subtitle})",
        x=0.01,
        ha="left",
    )
    save_figure(fig, outputs_dir / "finite_horizon_forgetting_simulation_by_period.png")
...
def plot_forgetting_effective_observations(
    ...
    fig.suptitle(
        f"Finite-horizon forgetting memory stock ({subtitle})",
        x=0.01,
        ha="left",
    )
    save_figure(
        fig,
        outputs_dir / "finite_horizon_forgetting_effective_observations.png",
    )


def plot_forgetting_representative_path(
    paths: pd.DataFrame,
    outputs_dir: Path,
) -> None:
    if paths.empty:
    ...
    decays = np.array(sorted(paths["forgetting_decay"].unique()))
    target_p0 = p0_values[(len(p0_values) - 1) // 2]
    target_decay = decays[-1]
    subset = paths[
        np.isclose(paths["p0"], target_p0)
        & np.isclose(paths["forgetting_decay"], target_decay)
    ].copy()
    if subset.empty:
        return

    def action_switch_count(actions: pd.Series) -> int:
        return int(actions.ne(actions.shift()).sum() - 1)

    switch_counts = (
        subset.sort_values(["rep", "t"])
        .groupby("rep", observed=True)["action_if_A"]
        .apply(action_switch_count)
        .sort_values(ascending=False)
    )
    target_rep = int(switch_counts.index[0])
    rep_data = subset[subset["rep"] == target_rep].sort_values("t")

    fig, axes = plt.subplots(3, 1, figsize=(10.8, 7.8), sharex=True)
    ax_demand, ax_mean, ax_action = axes

    ax_demand.plot(rep_data["t"], rep_data["rho"], color="#2563eb", linewidth=2.0)
    ax_demand.set_title("Demand probability", loc="left")
    ...
    ax_mean.plot(
        rep_data["t"],
        rep_data["posterior_mean"],
        color="#0f766e",
        linewidth=2.0,
    )
    ax_mean.set_title("Discounted posterior mean", loc="left")
    ax_mean.set_ylabel("E[theta | S,F]")
    ax_mean.set_ylim(-0.03, 1.03)
    prettify_axes(ax_mean)
    ...
    ax_action.step(
        rep_data["t"],
        rep_data["action_if_A"],
        where="post",
        color="#7c2d12",
        linewidth=2.0,
    )
    chosen = rep_data[rep_data["user_chose_A"] == 1]
    ax_action.scatter(
        chosen["t"],
        chosen["action_if_A"],
        color="#111827",
    ...
    ax_action.set_title("Seller action if A is chosen", loc="left")
    ax_action.set_xlabel("Period")
    ...
    fig.suptitle(
        "Representative forgetting path "
        f"(p0={target_p0:.2f}, lambda={target_decay:.3f}, rep={target_rep})",
        x=0.01,
        ha="left",
    )
    save_figure(fig, outputs_dir / "finite_horizon_forgetting_representative_path.png")
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Also run an approximate finite-horizon experiment where user beliefs "
            "use exponentially discounted pseudo-counts."
        ),
    )
    parser.add_argument(
        "--forgetting-decays",
        ...
        help=(
            "Comma-separated discount factors lambda in (0,1) for the optional "
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.5,
        help="Grid size for discounted pseudo-counts in the forgetting experiment.",
    )
    parser.add_argument(
        "--outputs-dir",
        default=None,
...
def validate_args(args: argparse.Namespace) -> None:
    ...
    if args.run_forgetting_experiment and args.user_policy != THOMPSON_USER_POLICY:
        raise ValueError(
            "The optional forgetting experiment currently supports only "
...
def main() -> None:
    ...
    forgetting_plots_dir = outputs_dir / "plots_forgetting"
    data_dir.mkdir(parents=True, exist_ok=True)
    plots_dir.mkdir(parents=True, exist_ok=True)
    if args.run_forgetting_experiment:
        forgetting_plots_dir.mkdir(parents=True, exist_ok=True)
    ...
    if args.run_forgetting_experiment:
        forgetting_summary_rows = []
        forgetting_usage_frames = []
        forgetting_path_frames = []
        forgetting_time_frames = []
        print(
            "\nRunning forgetting experiment with decays: "
            + ", ".join(f"{decay:.3f}" for decay in forgetting_decays)
        )
        print(
            "  Approximate pseudo-count grid step="
            f"{args.forgetting_grid_step:.4g}; decay applies every calendar period."
        )
        for decay_idx, decay in enumerate(forgetting_decays):
            for p0_idx, p0 in enumerate(p0_grid):
                print(
                    f"  lambda={decay:.3f}, p0={p0:.3f}: "
                    "solving approximate forgetting DP..."
                )
                forgetting_solution = solve_finite_horizon_forgetting_approx(
                    p0=float(p0),
                    params=params,
                    horizon=args.horizon,
                    early_periods=args.early_periods,
                ...
                forgetting_summary_rows.append(
                    summarize_forgetting_solution(forgetting_solution)
                )
                forgetting_usage_frames.append(forgetting_solution["usage_by_time"])

                if not args.skip_simulation:
                    paths, time_summary = simulate_forgetting_policy(
                    ...
                    forgetting_path_frames.append(paths)
                    forgetting_time_frames.append(time_summary)

        forgetting_summary = pd.DataFrame.from_records(forgetting_summary_rows)
        forgetting_usage = pd.concat(forgetting_usage_frames, ignore_index=True)
        forgetting_summary.to_csv(
            data_dir / "forgetting_experiment_summary.csv",
            index=False,
        )
        forgetting_usage.to_csv(
            data_dir / "forgetting_experiment_policy_by_time.csv",
            index=False,
        )

        if forgetting_path_frames:
            ...
            plot_forgetting_simulation_comparison(
                forgetting_times,
                forgetting_plots_dir,
            )
            plot_forgetting_effective_observations(
                forgetting_times,
                forgetting_plots_dir,
            )
            plot_forgetting_representative_path(
                forgetting_paths,
                forgetting_plots_dir,
            )
            plot_forgetting_posterior_density_evolution(
                forgetting_paths,
                forgetting_plots_dir,
            )
    ...
    if not forgetting_summary.empty:
        forgetting_display_columns = [
            ...
            "avg_value_T",
            "initial_action",
            "early_product_2_share_mean",
            "early_demand_weighted_product_2_share_mean",
        ]
        print("\nForgetting experiment summary")
        print(
    ...
    if args.run_forgetting_experiment:
        print(f"Saved forgetting plots to: {forgetting_plots_dir}")
