...
```bash
uv run python average_cost/main.py
```
...
- `average_cost/outputs/data/`
- `average_cost/outputs/plots/`

For a quick smoke test:

```bash
uv run python average_cost/main.py --p0-count 3 --max-observations 25 --T 40 --n-rep 20 --outputs-dir /tmp/llm_learning_average_cost_smoke
```

To run the solver without simulations:

```bash
uv run python average_cost/main.py --skip-simulation
```
...
```bash
uv run python average_cost/main.py --method projection
```
...
```bash
uv run python average_cost/main.py --sample-path-p0-grid 0.4,0.6 --sample-path-count 5
```

For a denser, slower numerical sweep:

```bash
uv run python average_cost/main.py --max-observations 50 --p0-count 17
```
...
```bash
uv run python average_cost/main.py --run-robustness
```
...
Robustness outputs are written under `average_cost/outputs/robustness/`, for example:

- `average_cost/outputs/robustness/data/method_comparison.csv`
- `average_cost/outputs/robustness/data/path_weighted_comparison.csv`
- `average_cost/outputs/robustness/data/finite_horizon_by_time_remaining.csv`
- `average_cost/outputs/robustness/data/policy_agreement_summary.csv`
- `average_cost/outputs/robustness/data/bias_threshold_diagnostic.csv`
- `average_cost/outputs/robustness/data/boundary_drift_diagnostic.csv`
- `average_cost/outputs/robustness/diagnostic_report.md`
- `average_cost/outputs/robustness/plots/method_comparison_dashboard_extended.png`
- `average_cost/outputs/robustness/plots/product2_region_comparison_extended_N=25.png`
- `average_cost/outputs/robustness/plots/bias_threshold_diagnostic_extended.png`
- `average_cost/outputs/robustness/plots/boundary_drift_diagnostic_extended_N=25.png`
- `average_cost/outputs/robustness/finite_horizon_T=25/plots/finite_horizon_product2_usage_all_p0.png`
- `average_cost/outputs/robustness/finite_horizon_T=25/plots/finite_horizon_time_remaining_extended.png`
- `average_cost/outputs/robustness/rolling_window_N=25/data/policy_by_state.csv`
- `average_cost/outputs/robustness/projection_N=25/data/policy_by_state.csv`
- `average_cost/outputs/robustness/stochastic_projection_N=25/data/policy_by_state.csv`
- `average_cost/outputs/robustness/finite_horizon_T=25/data/finite_horizon_policy.csv`
...
Data files are written to `average_cost/outputs/data/`:
...
Plots are written to `average_cost/outputs/plots/`:
...
The model writeup for this version is in `average_cost/Paper/main.tex`.
Compile it from the paper folder:

```bash
cd average_cost/Paper
