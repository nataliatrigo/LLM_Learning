...
def plot_simulation_timeseries(
    ...
    demand_title = "Simulated demand path"
    product_title = "Simulated product 2 use when A is chosen"
    if initial_belief_mean is not None:
        demand_title += f" (initial mean={initial_belief_mean:.2f})"
        product_title += f" (initial mean={initial_belief_mean:.2f})"

    axes[0].set_title(demand_title, loc="left")
    ...
    axes[1].set_title(product_title, loc="left")
...
def plot_user_belief_timeseries(
    ...
    suffix = f"_{filename_suffix}" if filename_suffix else ""
    save_figure(fig, outputs_dir / f"simulation_user_belief_paths_by_p0{suffix}.png")
...
def plot_user_belief_policy_comparison(
    ...
    if initial_belief_mean is not None:
        ax_mean.axhline(
            ...
            linewidth=1.1,
            linestyle=":",
            label=f"initial mean={initial_belief_mean:.2f}",
        )
    ...
    if initial_belief_mean is not None:
        title += f", initial mean={initial_belief_mean:.2f}"
    fig.suptitle(title, x=0.01, ha="left")
    suffix = f"_{filename_suffix}" if filename_suffix else ""
    filename = (
        "user_belief_optimal_vs_always_product2_"
        f"p0_{p0_filename_fragment(target_p0)}{suffix}.png"
    )
    save_figure(fig, outputs_dir / filename)
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--outputs-dir",
        default=None,
        help="Defaults to discounted/outputs next to this script.",
    )
...
def main() -> None:
    ...
    if target_solution is not None:
        ...
        if target_simulation_time is not None and target_simulation_seed is not None:
            ...
            filename_suffix = ""
            plot_initial_mean = None
            if initial_successes or initial_failures:
                plot_initial_mean = initial_belief_mean
                filename_suffix = (
                    "initmean_"
                    f"{p0_filename_fragment(initial_belief_mean)}"
                )
            plot_user_belief_policy_comparison(
                ...
                initial_belief_mean=plot_initial_mean,
