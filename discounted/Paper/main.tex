...
\begin{equation}
    ...
    \triangleq
    (R-c_x)
    + \gamma \big[ p_x V(S+1,F) + (1-p_x)V(S,F+1) \big].
\end{equation}
...
\begin{equation}
\begin{aligned}
    V(S, F) =& \big[1 - \rho(S, F)\big] \gamma V(S, F) + \rho(S, F) \max_{x \in \{1, 2\}} Q_x(S,F).
\end{aligned}
\end{equation}
...
\begin{equation}
    V(S, F) = \frac{\rho(S, F)}{1 - \gamma + \gamma \rho(S, F)} \max_{x \in \{1, 2\}} Q_x(S,F).
\end{equation}
